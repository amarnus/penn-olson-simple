		...
		$('.entry > .title > a').live('click', function(evt) {
			...
			if (eid) {
				...
				if (!$('#' + id).length) {
					...
					modal += '<div class="modal-footer">' + '<a class="btn btn-primary twitter-share-button" href="http://twitter.com/intent/tweet?text=' + encodeURIComponent(ENTRIES[eid].title + ' ' + ENTRIES[eid].link) + '&related=pennolson,amarnus'">Share on Twitter</a>'  + '</div>';
					modal += '</div>';
					$('body').append(modal);
